...
const LinksList = () => {
  const links = [
    ...
    {
      name: "baris-yildiz-cv",
      url: "/bariscv2026.pdf",
      svg: (
        <div className="  justify-evenly rounded-md bg-opacity-100 items-center flex animate-pulse border-white border">
          <svg
            width="24px"
            height="28px"
            viewBox="0 0 24 24"
      ...
      {links.map((link, index) => (
        ...
        >
          <motion.a
            ...
            initial="hidden"
            animate="visible"
            className={`flex space-x-1 text-slate-400 items-center cursor-pointer ${
              link.name === "baris-yildiz-cv"
                ? "w-full sm:w-14"
                : "w-7 h-7"
            }`}
            href={link.url}
            {...(link.name === "baris-yildiz-cv"
              ? { download: true }
              : { target: "_blank", rel: "noopener noreferrer", width: "28", height: "28" })}
          >
            {link.svg}
          </motion.a>
        </motion.span>
