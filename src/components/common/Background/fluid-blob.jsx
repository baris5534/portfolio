...
void main() {
    ...
    vec3 color = vec3(0.2, 0.5, 1.0); // <-- Sembolün rengi (mavi)
    ...
    if (t > 0.0) {
        ...
        color = vec3(fresnel) * vec3(0.2, 0.5, 1.0); // <-- Sembolün rengi (mavi)
        gl_FragColor = vec4(color, 1.0);
    } else {
        gl_FragColor = vec4(0.05, 0.05, 0.1, 1.0); // <-- Arka plan rengi (koyu lacivert)
    }
}
`;
  ...
  return (
    <div
      style={{
        width: "100%",
        height: "100%",
        background: "#05051a", // <-- Arka plan rengi (koyu lacivert)
        position: "absolute",
      }}
    >
      <Canvas
